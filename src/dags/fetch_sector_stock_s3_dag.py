...
def _fetch_and_upload_sector_data_from_df_api(
    s3_bucket, s3_base_prefix, sector_name, aws_conn_id, **kwargs
) -> None:
    """
    ...
    try:
        ...
        if check_result and check_result.get("exists"):
            price_df = yf.Sector(sector_name).ticker.history(
                period="1d", interval="15m"
            )
            price_df["Sector"] = sector_name
            target_time = current_time.astimezone(
                price_df.tail(1).index[0].tz
            ) - timedelta(minutes=19)

            # 목표로 하는 데이터만 남김
            price_df = price_df.loc[
                price_df.index.strftime("%Y-%m-%d %H-%M")
                == target_time.strftime("%Y-%m-%d %H-%M")
            ]

            if price_df.empty:
                log.warning(
                    f"[{sector_name}] Fetched DataFrame is empty. Skipping Parquet upload."
                )
                raise DataNotFoundException(
                    f"[{sector_name}] Data for {target_time.strftime('%Y-%m-%d %H%M')} not found."
                )
            elif target_time.strftime("%Y-%m-%d %H%M") == price_df.tail(1).index[
                0
            ].strftime("%Y-%m-%d %H%M"):
                price_df = price_df.tail(1)
            elif target_time.strftime("%Y-%m-%d %H%M") != price_df.index[0].strftime(
                "%Y-%m-%d %H%M"
            ):
                log.warning(
                    f"{sector_name}의 {target_time.strftime('%Y-%m-%d %H%M')} 데이터가 없습니다."
                )
                raise DataNotFoundException(
                    f"[{sector_name}] Data for {target_time.strftime('%Y-%m-%d %H%M')} not found."
                )
...
with DAG(
    dag_id="fetch_sector_stock_s3_dag",
    start_date=datetime(2025, 11, 1),
    schedule_interval="4,19,34,49 * * * *",
    catchup=False,
    tags=["s3", "sector", "stock"],
    default_args={
        "owner": "airflow",
