...
def _fetch_and_upload_sector_data_from_df_api(s3_bucket, s3_base_prefix, company_symbols, aws_conn_id, **kwargs) -> None:
    """
    ...
    check_result = context['ti'].xcom_pull(task_ids='check_s3_folder', key='return_value')
    ...
    try:
        ...
        for company_symbol in company_symbols:
            try:
                ...
                mtal_df = price_df.loc[:, price_df.columns.get_level_values('Ticker') == company_symbol]
                mtal_df.columns = mtal_df.columns.get_level_values('Price')
                mtal_df.index.name = 'Datetime'
                ohlcv_df = mtal_df[['Open', 'High', 'Low', 'Close', 'Volume', 'Dividends', 'Stock Splits']].copy()
                ...
                if term == '1d':
                    ...
                    target_time = current_time.astimezone(new_york_tz) - timedelta(minutes=19)
                    ohlcv_df = ohlcv_df.loc[ohlcv_df.index.strftime('%Y-%m-%d %H-%M') == target_time.strftime('%Y-%m-%d %H-%M')]
                else:
                ...
                if ohlcv_df.empty:
                    ...
                    raise DataNotFoundException(f"[{company_symbol}] Data for {target_time.strftime('%Y-%m-%d %H%M')} not found.")
                elif target_time.strftime('%Y-%m-%d %H%M') != ohlcv_df.tail(1).index[0].strftime('%Y-%m-%d %H%M'):
                    log.warning(f"{company_symbol}의 {target_time} 데이터가 없습니다. ")
                    raise DataNotFoundException(f"[{company_symbol}] Data for {target_time} not found.")
                ...
                with BytesIO() as parquet_buffer:
                    ohlcv_df.to_parquet(parquet_buffer, engine='pyarrow', compression='snappy')
                    ...
                    log.info(f"[{company_symbol}] Successfully uploaded Parquet data to S3: {s3_key}")
